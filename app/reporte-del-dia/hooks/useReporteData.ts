...
export const obtenerIniciales = (nombre: string): string => {
  ...
  if (palabras.length >= 2) {
    return (palabras[1].charAt(0) + palabras[2]?.charAt(0) || palabras[1].charAt(0)).toUpperCase();
  }
  return nombre.substring(0, 2).toUpperCase();
};

export const calcularProgresoUsuario = (estadisticas: { totalTareas: number; tareasTerminadas: number }): number => {
