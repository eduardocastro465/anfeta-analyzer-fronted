...
import { ApiResponse, Usuario, Actividad, DetalleView, ActividadConUsuario } from "../types/reporteTypes";
...
interface DashboardViewProps {
  ...
  currentView: DetalleView;
  selectedUser: Usuario | null;
  selectedActivity: Actividad | null;
  onNavigate: (view: DetalleView, user?: Usuario, activity?: Actividad) => void;
}
...
export default function DashboardView({
  ...
  const allActivities = useMemo((): ActividadConUsuario[] => {
  ...
  const filtrarActividadesPorFecha = (actividades: ActividadConUsuario[]) => {
    switch (fechaFiltro) {
      case "hoy":
  ...
  const handleViewUser = (usuario: Usuario) => {
    console.log("🚀 Navegando a usuario:", usuario.nombre);
    onNavigate('usuario', usuario);
  };

  const handleViewActivity = (actividad: Actividad, usuario: Usuario) => {
    console.log("🚀 Navegando a actividad:", actividad.titulo);
    onNavigate('actividad', usuario, actividad);
  };
  ...
  // Si estamos en una vista diferente al dashboard, mostrar DetallesView
  if (currentView !== 'dashboard') {
    return (
      <div className="font-arial p-6">
        <Button
          onClick={handleBackToDashboard}
          variant="outline"
          className="mb-6 border-none text-gray-300 hover:bg-gray-800 hover:text-white hover:border-purple-500/50"
        >
        ...
        <DetallesView
          detalleView={currentView}
          datos={{ estadisticas, data }}
          selectedUser={selectedUser}
          selectedActivity={selectedActivity}
          selectedTask={null}
          onBackToGeneral={handleBackToDashboard}
          onBackToUser={() => onNavigate('usuario', selectedUser || undefined)}
          onViewActivity={handleViewActivity}
          onViewTask={(tarea, actividad, usuario) => {
            console.log("Ver tarea:", tarea.nombre);
            // Puedes implementar navegación a tarea aquí si necesitas
          }}
  ...
  return (
    <div className="font-arial space-y-6">
                      ...
                      return (
                        <React.Fragment key={activity.actividadId}>
                          <TableRow className="bg-[#1a1a1a] border-b border-gray-800 hover:bg-[#1f1f1f]">
                            ...
                            <TableCell>
                              <div className="flex items-center gap-3">
                                <Avatar className="h-8 w-8 cursor-pointer hover:opacity-80"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleViewUser(activity.usuario);
                                  }}>
                            ...
                            <TableCell>
                              <Badge className={`${activity.status === 'completada'
                            ...
                            <TableCell className="text-right">
                              <div className="flex justify-end gap-1">
                                ...
                                <Button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    console.log("👁️ Botón Eye clickeado - Actividad:", activity.titulo);
                                    handleViewActivity(activity, activity.usuario);
                                  }}
                                ...
                                <div className="p-4">
                                  <div className="flex items-center justify-between mb-3">
                                    ...
                                    <Button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        console.log("📋 Ver todas las tareas - Actividad:", activity.titulo);
                                        handleViewActivity(activity, activity.usuario);
                                      }}
                                        ...
                                        {activity.pendientes.slice(0, 4).map((tarea, idx) => (
                                          <tr key={idx} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                                            <td className="py-2 px-3">
                                              <div className="flex items-center gap-2">
                                                <div className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${tarea.terminada
