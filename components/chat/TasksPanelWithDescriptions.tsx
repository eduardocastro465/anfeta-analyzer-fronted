...
export function TasksPanelWithDescriptions({
  ...
  const estadisticas = useMemo(
    () => ({
      totalTareas: actividadesConDescripcion.reduce(
        (s, a) => s + a.tareasConDescripcion.length,
        0,
      ),
      tareasSinDescripcion: actividadesConDescripcion.reduce(
        (s, a) => s + (a.tareasSinDescripcion || 0),
        0,
      ),
      tareasBloqueadas: actividadesConDescripcion.reduce(
        (s, a) => s + (a.tareasConDescripcionCount || 0),
        0,
      ),
    ...
    <div className="w-full animate-in slide-in-from-bottom-2 duration-300">
      ...
      {mostrarAlerta && (
        <div className="fixed top-3 right-3 z-50 animate-in slide-in-from-right duration-300">
          <div
            className={`px-3 py-2 rounded-lg shadow-lg flex items-center gap-2 text-xs ${theme === "dark" ? "bg-gray-800 text-white border border-gray-700" : "bg-white text-gray-800 border border-gray-200 shadow-md"}`}
          >
            ...
            <Button
              variant="ghost"
              size="sm"
              className={`ml-1 p-1 h-auto ${theme === "dark" ? "text-gray-400 hover:text-white" : "text-gray-500 hover:text-gray-800"}`}
      ...
      {hayTareas ? (
        <div
          className={`w-full max-w-xl rounded-lg border overflow-hidden ${theme === "dark" ? "bg-[#1a1a1a] border-[#2a2a2a]" : "bg-white border-gray-200 shadow-sm"}`}
        >
          {/* Header */}
          <div
          ...
          >
            <div className="flex justify-between items-center">
              ...
              >
                <FileText
                  className={`w-3.5 h-3.5 ${esHistorial ? "text-gray-500" : "text-[#6841ea]"}`}
                />
                Tareas del Día ({estadisticas.totalTareas})
                {esHistorial && (
                  <span
                    className={`text-[10px] px-1.5 py-0.5 rounded font-medium normal-case tracking-normal flex items-center gap-1 ${
                      theme === "dark"
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              {estadisticas.tareasBloqueadas > 0 && (
                ...
                >
                  ...
                  {estadisticas.tareasBloqueadas}{" "}
                  {esHistorial ? "Registradas" : "Completadas"}
                </UIBadge>
              )}
              {!esHistorial && (
                <UIBadge
            ...
            >
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <strong>
                  {esHistorial
                    ...
                    : `${estadisticas.tareasSinDescripcion} tareas necesitan descripción`}
                </strong>
                {!esHistorial && (
                  <span
                    className={`text-[10px] px-1.5 py-0.5 rounded ${theme === "dark" ? "bg-amber-500/20 text-amber-300" : "bg-amber-500/15 text-amber-700"}`}
                  >
                    Modo Voz
                  </span>
                )}
              ...
              <span className="block mt-1 opacity-80">
                ...
                {estadisticas.tareasBloqueadas > 0 && (
                  ...
                  >
                    {estadisticas.tareasBloqueadas}{" "}
                    {esHistorial ? "con descripción" : "ya completadas"}
                  </span>
                )}
              </span>
            </div>
...
function ActivityWithDescriptionItem({
  ...
  return (
    <div
      className={`p-2 rounded-lg border ${theme === "dark" ? "bg-[#252527] border-[#333335]" : "bg-gray-50 border-gray-200"}`}
    >
      {/* Header de actividad */}
      <div className="flex items-start justify-between mb-1.5 gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <div
            className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${badgeColor}`}
          >
            {index + 1}
          </div>
          <div className="min-w-0">
            <h5
              className={`font-medium text-xs line-clamp-2 ${theme === "dark" ? "text-gray-200" : "text-gray-800"}`}
            >
              {actividad.titulo}
            </h5>
            <span
              className={`text-[10px] ${theme === "dark" ? "text-blue-400" : "text-blue-600"}`}
            >
        ...
        <UIBadge
          variant="outline"
          className={`text-[10px] shrink-0 ${theme === "dark" ? "border-[#3a3a3a] text-gray-400 bg-[#1a1a1a]" : "border-gray-300 text-gray-600 bg-white"}`}
        >
          {actividad.horario}
        </UIBadge>
      </div>
      ...
      <div className="ml-7 mb-2">
        {esActividadIndividual ? (
          <UIBadge
            variant="secondary"
            className={`text-[10px] px-1.5 py-0.5 flex items-center gap-1 w-fit ${theme === "dark" ? "bg-blue-500/20 text-blue-300 border border-blue-500/20" : "bg-blue-100 text-blue-700 border border-blue-200"}`}
          >
          ...
          <UIBadge
            variant="secondary"
            className={`text-[10px] px-1.5 py-0.5 flex items-center gap-1 w-fit ${theme === "dark" ? "bg-green-500/20 text-green-300 border border-green-500/20" : "bg-green-100 text-green-700 border border-green-200"}`}
          >
        ...
        {revision.tareasConDescripcion.filter((t: any) => !t.tieneDescripcion)
          ...
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              <UIBadge
                variant="outline"
                className={`text-[10px] ${theme === "dark" ? "bg-amber-500/20 text-amber-300 border-amber-500/30" : "bg-amber-50 text-amber-700 border-amber-300"}`}
              >
        ...
        {revision.tareasConDescripcion.filter((t: any) => t.tieneDescripcion)
          ...
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              <UIBadge
                variant="outline"
                className={`text-[10px] ${theme === "dark" ? "bg-green-500/20 text-green-300 border-green-500/30" : "bg-green-50 text-green-700 border-green-300"}`}
              >
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2">
        {/* Checkbox */}
        <div
          className={`flex items-center mt-0.5 ${tarea.bloqueada || esHistorial ? "cursor-not-allowed" : "cursor-pointer"}`}
          onClick={(e) => {
            e.stopPropagation();
            if (!tarea.bloqueada && !esHistorial) onToggleSeleccion();
          }}
        >
          <div
            className={`w-4 h-4 flex items-center justify-center border rounded transition-all ${
              tarea.bloqueada
                ? theme === "dark"
                ...
                : estaSeleccionada
                  ...
                  : theme === "dark"
                    ? "border-gray-600 hover:border-[#6841ea]"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-start justify-between gap-2">
              <span
                className={`text-xs line-clamp-2 ${tarea.bloqueada ? (theme === "dark" ? "text-green-300" : "text-green-700") : theme === "dark" ? "text-gray-200" : "text-gray-800"}`}
              >
                {tarea.nombre}
              </span>
              <UIBadge
              ...
              {tarea.bloqueada ? (
                ...
                <UIBadge
                  className={`text-[10px] px-1.5 py-0.5 ${
                    esHistorial
                      ? theme === "dark"
                        ? "bg-gray-700/50 text-gray-600 border border-gray-600/50"
                        : "bg-gray-100 text-gray-400 border border-gray-300"
                      : theme === "dark"
                ...
                >
                  ...
                  {esHistorial ? "SIN REGISTRAR" : "SIN DESCRIPCIÓN"}
                </UIBadge>
              )}
            </div>
            ...
            {tarea.descripcion && (
              <div className="mt-1">
                ...
                {mostrarDescripcion && (
                  <div
                    className={`mt-1 p-1.5 rounded text-[10px] ${theme === "dark" ? "bg-[#2a2a2a] text-gray-300 border border-[#3a3a3a]" : "bg-gray-100 text-gray-700 border border-gray-200"}`}
                  >
...
function DescriptionTasksFooter({
  ...
  if (esHistorial) {
    return (
      <div
        className={`px-3 py-2.5 border-t ${theme === "dark" ? "border-[#2a2a2a] bg-[#1a1a1a]" : "border-gray-200 bg-gray-50"}`}
      >
        <p
  ...
  return (
    <div
      className={`p-2 border-t ${theme === "dark" ? "border-[#2a2a2a] bg-[#202022]" : "border-gray-200 bg-gray-50"}`}
    >
      <div className="flex flex-col gap-2">
        <div className="flex justify-between items-center text-xs">
          ...
          <UIBadge
            variant="outline"
            className={`text-[10px] px-1.5 py-0.5 flex items-center gap-1 ${theme === "dark" ? "bg-purple-500/15 text-purple-300 border-purple-500/30" : "bg-purple-50 text-purple-700 border-purple-300"}`}
          >
        ...
        {totalTareas > 0 && (
          <Button
            ...
            disabled={isLoading || esHistorial}
            size="sm"
            variant="outline"
        ...
        <Button
          ...
          disabled={countSeleccionadas === 0 || isLoading || esHistorial}
        >
          {isLoading ? (
            <>
...
function NoDescriptionTasksMessage({
  ...
  return (
    <div className="animate-in slide-in-from-bottom-2 duration-300 flex justify-center">
      <div
        className={`p-4 rounded-lg border text-center ${theme === "dark" ? "bg-[#1a1a1a] border-[#2a2a2a]" : "bg-white border-gray-200 shadow-sm"}`}
      >
        <div
          className={`w-10 h-10 rounded-full flex items-center justify-center mx-auto mb-2 ${theme === "dark" ? "bg-green-500/15" : "bg-green-100"}`}
        >
          <Check className="w-5 h-5 text-green-500" />
        </div>
        <h4
          className={`font-semibold mb-1 text-xs ${theme === "dark" ? "text-gray-200" : "text-gray-800"}`}
        >
          ¡Todo listo!
        </h4>
        <p
          className={`text-[11px] ${theme === "dark" ? "text-gray-500" : "text-gray-500"}`}
        >
          {currentUserEmail.split("@")[0]}, todas tus tareas ya tienen
          descripción. ¡Buen trabajo!
        </p>
      </div>
    </div>
  );
