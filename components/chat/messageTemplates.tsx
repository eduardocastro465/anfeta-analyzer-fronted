...
function NoTasksFound({ onRefrescar }: { onRefrescar?: () => void }) {
  const theme = useTheme();
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p
            className={`text-[11px] mt-0.5 mb-2 ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}
          >
          ...
          {onRefrescar && (
            <button
              ...
              className={`flex items-center gap-1.5 text-[11px] font-medium px-2 py-1 rounded-md border transition-colors ${
                theme === "dark"
                  ? "border-gray-600 text-gray-400 hover:text-gray-200 hover:bg-gray-700 disabled:opacity-50"
                  : "border-gray-300 text-gray-500 hover:text-gray-700 hover:bg-gray-100 disabled:opacity-50"
              }`}
            >
              <RefreshCw
...
export const tasksTemplates = {
  ...
  noTasksFound: ({ onRefrescar }: { onRefrescar?: () => void } = {}) => (
    <NoTasksFound onRefrescar={onRefrescar} />
  ),
};
