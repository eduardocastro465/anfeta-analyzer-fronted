...
export function TasksPanelContent({
  ...
  const procesarTareasReportadas = useCallback(
    (nuevasTareasReportadas: any[], metadata: any = null) => {
      console.log(
        "Procesando NUEVAS tareas reportadas:",
        nuevasTareasReportadas.length,
      );
      ...
      if (metadata) {
        ...
        if (!tieneReportesPropios && tieneReportesColaborativos) {
          ...
          console.log(
            "Mostrando reportes de otros colaboradores (no tiene propios)",
          );
        } else {
      ...
      if (nuevasTareasReportadas.length === 0) {
        console.log("No hay nuevas tareas para procesar");
        return;
      }
        ...
        nuevasTareasReportadas.forEach((item: any, index: number) => {
          ...
          if (tareaExistente) {
            ...
            if (fechaNueva > fechaExistente) {
              ...
              actualizadas++;
              console.log(
                `Tarea ${tareaId} ACTUALIZADA (Reportado por: ${reportadoPor}, Es mío: ${esMiReporte})`,
              );
            } else {
              console.log(
                `Tarea ${tareaId} ya existe (más reciente), omitiendo`,
              );
            }
          } else {
            nuevoMap.set(tareaId, {
            ...
            nuevasAgregadas++;
            console.log(
              `Tarea ${tareaId} AGREGADA (Reportado por: ${reportadoPor}, Es mío: ${esMiReporte})`,
            );
          }
        });

        const misReportes = Array.from(nuevoMap.values()).filter(
          (t) => t.esMiReporte,
        ).length;
        const reportesDeOtros = Array.from(nuevoMap.values()).filter(
          (t) => !t.esMiReporte,
        ).length;
    ...
    async (esForzado: boolean = false) => {
      ...
      try {
        ...
        if (Array.isArray(tareasData) && tareasData.length > 0) {
          ...
          console.log("No se encontraron tareas reportadas");
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [currentUserEmail, cargarTareasReportadas]);
  ...
  const actividadesConTareas = useMemo(() => {
    ...
    console.log(
      "Procesando revisiones por actividad:",
      assistantAnalysis.data.revisionesPorActividad.length,
    );
  ...
  const toggleSeleccionTarea = useCallback(
    (tarea: any) => {
      ...
      if (reporte) {
        console.log("Tarea ya reportada:", reporte);
  ...
  const seleccionarTodasTareas = useCallback(() => {
    const todasTareasIds = actividadesConTareas.flatMap((actividad) =>
      actividad.tareasNoReportadas.map((t: any) => t.id),
    );

    console.log(
      "Seleccionando todas las tareas pendientes:",
      todasTareasIds.length,
    );
    ...
    mostrarAlertaMensaje(
      `${todasTareasIds.length} tareas pendientes seleccionadas`,
    );
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h4
                className={`font-medium text-sm mb-1 ${theme === "dark" ? "text-purple-300" : "text-purple-700"}`}
              >
                Reportes del equipo
              </h4>
              <p
                className={`text-xs mb-2 ${theme === "dark" ? "text-purple-200" : "text-purple-600"}`}
              >
                ...
                {mostrandoReportesDeOtros ? (
                  <>
                    ...
                    <strong>
                      ...
                      {estadisticasServidor?.tareasColaboradores || 0} reportes)
                    </strong>
                  </>
                ) : (
                  <>
                    ...
                    <strong>
                      Tareas Pendientes: {estadisticas.totalNoReportadas}
                    </strong>
                  </>
                )}
                {!mostrandoReportesDeOtros && (
                ...
                {mostrandoReportesDeOtros ? (
                  <>
                    ...
                    {currentUserEmail
                      ...
                      : "Usuario"}
                    <span className="ml-2">•</span> No tienes reportes propios,
                    pero hay reportes de otros colaboradores.
                  </>
                ) : (
                  <>
          ...
          <div className="flex items-center gap-2">
            {esActividadIndividual ? (
              ...
              >
                ...
                Individual
                <span className="text-[9px] opacity-75 ml-1">(Solo tú)</span>
              </Badge>
            ) : (
              <Badge
                variant="secondary"
...
function ReportedTaskItem({
  tarea,
  theme,
  reporteInfo,
  esMiReporte,
  currentUserEmail,
}: ReportedTaskItemProps) {
              ...
              <div className="mt-2">
                ...
                >
                  ...
                  {mostrarDescripcion
                    ...
                    : "Ver explicación"}
                  <ChevronDown
                    className={`w-3 h-3 ml-1 transition-transform ${mostrarDescripcion ? "rotate-180" : ""}`}
                  />
                </Button>
                ...
                {mostrarDescripcion && (
                  ...
                  >
                    ...
                    {reporteInfo.encontradoEn && (
                      <div
                        className={`mt-1 pt-1 border-t ${theme === "dark" ? "border-[#3a3a3a]" : "border-gray-300"}`}
                      >
                        <span
                          className={`text-[10px] ${theme === "dark" ? "text-gray-500" : "text-gray-600"}`}
                        >
