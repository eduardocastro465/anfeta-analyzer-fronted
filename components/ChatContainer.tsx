...
export function ChatContainer({
  ...
  const [keysRequeridas, setKeysRequeridas] = useState({
    groq: false,
    gemini: false,
  });
  const [keysExistentes, setKeysExistentes] = useState<{
    groq: string[];
    gemini: string;
  }>({ groq: [], gemini: "" });
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setKeysExistentes({ groq: [], gemini: "" });
      setKeysRequeridas({ groq: false, gemini: false });
      setShowKeysModal(false);

      const keysRes = await obtenerKeysUsuario();
      console.log("keysRes", keysRes);
      if (keysRes.success) {
        const faltaGroq = !keysRes.keys.groq || keysRes.keys.groq.length === 0;
        const faltaGemini = !keysRes.keys.gemini;

        setKeysExistentes({
          groq: keysRes.keys.groq || [],
          gemini: keysRes.keys.gemini || "",
        });

        if (faltaGroq) {
          setKeysRequeridas({ groq: faltaGroq, gemini: faltaGemini });
      ...
      if (response.success && response.data) {
        ...
        if (!data.ultimoAnalisis) {
          ...
          try {
            ...
            console.log("dataFresca", dataFresca);
  ...
  return (
    ...
    `}
      ...
      <ApiKeysModal
        open={showKeysModal}
        theme={theme}
        keysRequeridas={keysRequeridas}
        keysIniciales={keysExistentes}
